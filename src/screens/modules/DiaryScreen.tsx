...
const ACCENT = '#0ea5e9';
const WEATHER_OPTIONS = ['Sunny','Cloudy','Rainy','Windy','Stormy','Foggy','Snowy'];
...
export default function DiaryScreen() {
  ...
  const openDetail = async (entry: FullEntry) => {
    ...
    try {
      const res = await client.get(`/diary/${entry.id}`);
  ...
  const handleWorkflowAction = async (action: 'approve'|'reject'|'back') => {
      ...
      await new Promise<void>((resolve) => {
        Alert.prompt(
          ...
          [
            ...
            { text: 'Submit', onPress: (val) => { comment = val || ''; resolve(); } },
          ],
          'plain-text',
        );
      });
    ...
    try {
      await client.put(`/diary/${selectedEntry.id}/update`, { action, comment, userId: user.id });
  ...
  const handleDelete = (entry: FullEntry) => {
    Alert.alert('Delete Entry', 'Are you sure? This cannot be undone.', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try {
          await client.delete(`/diary/${entry.id}`);
          setShowDetail(false);
          fetchEntries();
        ...
        <FlatList data={filtered} keyExtractor={i => i.id} renderItem={({ item }) => (
          <TouchableOpacity style={S.card} onPress={() => openDetail(item)} activeOpacity={0.7}>
            <View style={S.cardTop}>
              <View style={S.cardLeft}>
                ...
                <View style={[S.pill, { backgroundColor: ACCENT+'22' }]}>
                  <Icon name="weather-partly-cloudy" size={10} color={ACCENT} />
                  <Text style={[S.pillText, { color: ACCENT }]}>{item.weather}</Text>
                </View>
              </View>
            ...
            <View style={S.cardMeta}>
              <View style={S.metaItem}><Icon name="account-outline" size={12} color={colors.textMuted} /><Text style={S.metaText}>{item.author}</Text></View>
              {item.temperature ? <View style={S.metaItem}><Icon name="thermometer" size={12} color={colors.textMuted} /><Text style={S.metaText}>{item.temperature}</Text></View> : null}
              ...
              <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1 }}>
                ...
                {selectedEntry.diary_workflow_nodes && selectedEntry.diary_workflow_nodes.length > 0 && (
                  <View style={S.section}>
                    ...
                    {selectedEntry.diary_workflow_nodes.sort((a,b)=>a.node_order-b.node_order).map(node => (
          ...
          <View style={S.overlay}>
            <View style={S.createSheet}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <FF label="Temperature"><TextInput style={S.input} value={temperature} onChangeText={setTemperature} placeholder="e.g. 24°C" placeholderTextColor={colors.textMuted} /></FF>
                <FF label="Work Completed *"><TextInput style={[S.input,S.ta]} value={workCompleted} onChangeText={setWorkCompleted} placeholder="Describe work completed today..." placeholderTextColor={colors.textMuted} multiline numberOfLines={3} /></FF>
                <FF label="Incidents Reported"><TextInput style={[S.input,S.ta]} value={incidents} onChangeText={setIncidents} placeholder="Any incidents or safety concerns..." placeholderTextColor={colors.textMuted} multiline numberOfLines={2} /></FF>
                <FF label="Materials Delivered"><TextInput style={[S.input,S.ta]} value={materials} onChangeText={setMaterials} placeholder="Materials received on site..." placeholderTextColor={colors.textMuted} multiline numberOfLines={2} /></FF>
                ...
                <TouchableOpacity style={[S.submitBtn, { backgroundColor:ACCENT }, submitting && { opacity:0.6 }]} onPress={handleCreate} disabled={submitting}>
                  {submitting ? <ActivityIndicator color="#fff" size="small" /> : <><Icon name="send" size={16} color="#fff" /><Text style={S.submitText}>Submit Diary Entry</Text></>}
                </TouchableOpacity>
                <View style={{ height: 40 }} />
              </ScrollView>
            </View>
...
const S = StyleSheet.create({
  ...
  workText: { color:colors.text, fontSize:13, lineHeight:18 },
